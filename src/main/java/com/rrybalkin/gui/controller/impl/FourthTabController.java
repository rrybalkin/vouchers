...
public class FourthTabController extends AbstractController implements Initializable, Observer {
    ...
    private void printActualDataReport(ReportWriter writer) throws Exception {
        ...
        for (Visitor visitor : visitors) {
            ...
            if (visitor.getTalon().getDinners() > 0 || visitor.getTalon().getLunches() > 0) {
        ...
        ReportData allVisitorsData = new ActualReportData(visitors, false);
        ...
        for (Association group : reportedGroups) {
            ReportData reportData = new ActualReportData(visitorsWithGroup.get(group), false);
            File report;
     ...
     */
    private ReportData sortReportData(ReportData reportData) throws Exception {
        // sorting report data
